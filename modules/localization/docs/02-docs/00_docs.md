...
The line detector node is responsible for detecting lines in the field of view of the Duckiebot, of three different colors: red, white and yellow.

### Ros interfacing of the node

The `line_detector_node` subscribes to:

* The corrected image stream

The `line_detector_node` publishes:
...
The `lane_filter_node` subscribes to:

* The segment list from the line detector node

The `lane_filter_node` publishes:
...
Then, from the updated belief matrix, the maximum is extracted. The maximum's coordinates give us the best estimate of the tuple ($d$, $\phi$).
